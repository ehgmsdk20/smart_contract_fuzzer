        ...
        if func['name'] != 'constructor':
            for _ in range(num_cases):
                ...
                for param in func['parameters']:
                    ...
                    else: #int16 -> int24 mapping
                        params.append(Slither.MappingType(random_uint256(),random_uint256()))
    ...
    try:
        ...
        for contract in contracts:
            ...
            for func in contract.functions:
                ...
                param_list = []
                for param in func.parameters:
                    if hasattr(param.type, "type"): #type.type과 type(mapping) 분리
                        param_type = param.type.type 
                        param_list.append((param_type, param.name))
                    else :
                        param_list.append((param.type, param.name))
                if not func.view:
                    functions.append({
                        'name': func_name, 
...
def fuzz_contract(contract, functions):
    error_logs = []
    gas_usages = []
    test_cases = generate_test_cases(functions, num_cases=10)  
